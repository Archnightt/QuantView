...
export function MarketSummaryWidget({ data }: { data: any }) {
  ...
  return (
    <Card className="flex flex-col dark:bg-card overflow-hidden">
      {/* Status Bar */}
      ...
      <Tabs defaultValue="commodities" className="flex-1 flex flex-col">
        ...
        <div className="pt-2 pb-4 overflow-y-auto max-h-[350px] scrollbar-thin scrollbar-thumb-secondary">
          <TabsContent value="crypto" className="mt-0"><QuoteGrid items={data.crypto || []} /></TabsContent>
          <TabsContent value="rates" className="mt-0"><QuoteGrid items={data.rates || []} /></TabsContent>
          <TabsContent value="commodities" className="mt-0"><QuoteGrid items={data.commodities || []} /></TabsContent>
          <TabsContent value="currencies" className="mt-0"><QuoteGrid items={data.currencies || []} /></TabsContent>
        </div>
      </Tabs>
    </Card>
  );
