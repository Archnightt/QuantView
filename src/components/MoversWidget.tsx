    ...
    <div className="space-y-1 pr-1">
      {items.map((item, idx) => {
        ...
        return (
          ...
          >
            {/* Rank */}
            <span className="text-[10px] font-open text-muted-foreground/50 w-4 shrink-0 tabular">
              {idx + 1}
            </span>
            <div className="flex flex-col flex-1 min-w-0">
              {/* Ticker in mono */}
              <span className="font-open text-sm font-bold leading-tight tracking-tight">{item.symbol}</span>
            ...
            <div className="text-right shrink-0">
              ...
              {/* % in mono */}
              <div className={`flex items-center justify-end gap-0.5 font-open text-xs font-bold tabular mt-0.5 ${isGainer ? 'text-emerald-500' : 'text-red-500'
                }`}>
      ...
      <CardContent className="flex-1 p-3 pt-3 min-h-0 overflow-hidden">
        <Tabs defaultValue="gainers" className="w-full h-full flex flex-col">
          <TabsList className="grid w-full grid-cols-2 mb-3 shrink-0 bg-secondary/40 p-0.5 rounded-lg h-8">
            <TabsTrigger
              value="gainers"
              className="text-[11px] font-mono font-bold rounded-md h-full data-[state=active]:bg-emerald-500/15 data-[state=active]:text-emerald-500 text-muted-foreground"
            >
              ▲ Gainers
            </TabsTrigger>
            <TabsTrigger
              value="losers"
              className="text-[11px] font-mono font-bold rounded-md h-full data-[state=active]:bg-red-500/15 data-[state=active]:text-red-500 text-muted-foreground"
            >
              ▼ Losers
            </TabsTrigger>
          </TabsList>
